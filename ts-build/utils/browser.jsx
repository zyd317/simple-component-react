...
let ua = navigator.userAgent, platform = navigator.platform, android = ua.match(/(Android);?[\s\/]+([\d.]+)?/), // 匹配 android
...
let browsers = {
    ...
    weibo: ua.match(/(weibo__)([\d\.]+)/),
    uc: ua.match(/(UCBrower)\/([\d\.]+)/),
...
if (ipod) {
    ySniff.osVersion = ipod[3] ? ipod[3].replace(/_/g, '.') : '';
    ySniff.ipod = true;
    ySniff.imobile = true;
}
// iOS 8+ changed UA
...
if (ySniff.osVersion) {
    const ver = ySniff.osVersion.match(/\d+\.?\d*/);
    if (ver) {
        ySniff.osVersionN = parseInt(ver[0]);
    }
}
// 配置
