...
class ConfirmDialog extends Component {
    ...
    render() {
        ...
        if (contentType === 'confirm') {
            return (<Dialog title={title} close={handleClose} buttons={[
                { text: '确定', fn: handleSure },
                { text: '取消', fn: handleClose },
            ]}>{content}</Dialog>);
        }
        return (<Dialog title={title} showCloseIcon={false} close={handleClose} buttons={[{ text: '确定', fn: handleClose }]}>{content}</Dialog>);
    }
    open(config) {
